...
describe('fetchExchangeInfo', () => {
  it('should return a exchangeInfo from user', async () => {
    ...
    const hash = 'E829C12D96F65B23F60270767EEFCDC772BF4A5F756142F3944D908E78CA8516';
    ...
    const result = await fetchExchangeInfo(hash, momijiUserAccount, 'user');
    console.log(result);
  ...
  it('should return a exchangeInfo from seller', async () => {
    ...
    const result = await fetchExchangeInfo(hash, momijiSellerAccount, 'seller');
    console.log(result);
    expect(result).toHaveProperty('productInfo');
  }, 10000); // 10 seconds
  it('should return a exchangeInfo from admin', async () => {
    ...
    const result = await fetchExchangeInfo(hash, momijiAdminAccount, 'admin');
    console.log(result);
    expect(result).toHaveProperty('productInfo');
  }, 10000); // 10 seconds
  it('should throw an error if Failed to parse ExchangeOverview object', async () => {
    const momijiBlockChain = await setupBlockChain('momiji');
    const hash = '994FE428F123835127EC8B814C152804FA25B1CA33D2A4C6D77869390A6A568E';
    const momijiUserAccount = Account.createFromPrivateKey('85425CE250D4CFB863B2DC24327C7736F57AE8BB0FCD7E56D3FC504D97FBE556', momijiBlockChain.networkType);
    await expect(fetchExchangeInfo(hash, momijiUserAccount, 'user')).rejects.toThrow(
